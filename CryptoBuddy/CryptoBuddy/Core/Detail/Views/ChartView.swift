        ...
        GeometryReader { geo in
            Path { path in
                for index in data.indices {
                    let xPposition = geo.size.width / CGFloat(data.count) * CGFloat(index + 1)
                    
                    let yAxis = maxY - minY
                    let yPosition = (1 - CGFloat((data[index] - minY) / yAxis)) * geo.size.height
                    
                    if index == 0 {
                        path.move(to: CGPoint(x: xPposition, y: yPosition))
                    }
                    path.addLine(to: CGPoint(x: xPposition, y: yPosition))
                }
            }
